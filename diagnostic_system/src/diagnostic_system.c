...
/* EDL description of the diagnostic_system entity. */
#include <traffic_light/diagnostic_system.edl.h>
                          ...
                          struct nk_arena* res_arena)
{
     ...
     * sent to the client in the diagnostic_system response. */
...
/* diagnostic_system entry point. */
int main(void)
{
    NkKosTransport transport;
    ServiceId iid;

    /* Get diagnostic_system IPC handle of "diagnostic_system_connection". */
    ...
    /* Prepare the structures of the request to the diagnostic_system entity: constant
     * part and arena. Because none of the methods of the diagnostic_system entity has
     ...
     * diagnostic_system transport methods (nk_transport_recv, nk_transport_reply) and
     * to the diagnostic_system_entity_dispatch method. */
    traffic_light_diagnostic_system_entity_req req;
    char req_buffer[traffic_light_diagnostic_system_entity_req_arena_size];
    ...
    traffic_light_diagnostic_system_entity_res res;
    char res_buffer[traffic_light_diagnostic_system_entity_res_arena_size];
    ...
    /* Initialize diagnostic_system entity dispatcher. */
    traffic_light_diagnostic_system_entity entity;
    traffic_light_diagnostic_system_entity_init(&entity, &component);

    fprintf(stderr, "Hello I'm diagnostic_system\n");
    ...
    do
    {
        ...
        /* Wait for request to diagnostic_system entity. */
        if (nk_transport_recv(&transport.base, &req.base_, &req_arena) != NK_EOK) {
            ...
            traffic_light_diagnostic_system_entity_dispatch(&entity, &req.base_, &req_arena, &res.base_, &res_arena);
        }
